...
async function callApi(body: Record<string, unknown>): Promise<CanvasResponse> {
  return request<CanvasResponse>("POST", "/api/canvas", body);
}
...
function formatResult(data: CanvasResponse): string {
  ...
  if (data.edit_token) lines.push(`Edit token: ${data.edit_token}`);
...
server.tool(
  "create_diagram",
  "Render a Mermaid diagram as a pannable, zoomable canvas on gui.now and get a shareable URL",
  {
    mermaid: z
      .string()
  ...
  async ({ mermaid, title, theme, expires }) => {
    try {
      ...
      const body: Record<string, unknown> = { mermaid };
      if (title) body.title = title;
      if (theme) body.theme = theme;
      if (expires) body.expires = expires;
...
server.tool(
  "update_canvas",
  "Replace the content of an existing gui.now canvas. Needs the edit_token returned when it was created. Free tier allows 3 edits per canvas; Pro is unlimited. All viewers see the change immediately.",
  {
  ...
  async ({ canvas_id, edit_token, html, title, frames }) => {
    try {
      ...
      const data = await request<Record<string, unknown>>(
        "PUT",
        `/api/canvas/${encodeURIComponent(canvas_id)}`,
        body,
...
server.tool(
  "extend_canvas",
  "Push a gui.now canvas's expiry out to 24 hours from now, so a canvas that is about to lapse stays reachable",
  {
    canvas_id: z.string().describe("The canvas id, e.g. 'abc123'"),
  },
  async ({ canvas_id }) => {
    try {
      const data = await request<{ id: string; expires_at: string }>(
        "POST",
        `/api/canvas/${encodeURIComponent(canvas_id)}/extend`,
        {}
      );
      return {
        content: [
